...
#ifndef XP_BUTTON_H_INCLUDED
#define XP_BUTTON_H_INCLUDED
...
public:
    ...
    // Returns true if the variable has not changed state for at least
    // debounce time, false otherwise.
...
private:
    uint8_t m_pin;          // GPIO pin number connected to button
    ...
    bool m_stable;          // true if the variable state has not changed for at least debounce time
    bool m_lastTransient;   // holds variable state during debounce time
    uint32_t m_lastDbTime;  // last debounce time (ms)
